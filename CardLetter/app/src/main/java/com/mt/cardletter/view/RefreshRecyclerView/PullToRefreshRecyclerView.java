package com.mt.cardletter.view.RefreshRecyclerView;
...
//    private static final String TAG = "PTRRV";
    ...
    private void initView(){
        ...
        this.addView(mRootRelativeLayout);
                 /*下拉刷新的转圈圈的颜色*/
        this.setColorSchemeResources(R.color.swap_holo_bule_bright,R.color.swap_holo_green_bright,
                R.color.swap_holo_bule_bright,R.color.swap_holo_green_bright);
...
//            lastVisibleItem = mLinearLayoutManager.findLastCompletelyVisibleItemPosition();
    ...
    public void initRefreshView(Activity mActivity,RecyclerView.LayoutManager layoutManager){
        this.setSwipeEnable(true);//open swipe
        DemoLoadMoreView loadMoreView = new DemoLoadMoreView(mActivity, this.getRecyclerView());
        loadMoreView.setLoadmoreString(mActivity.getString(R.string.loading));//正在加载中
        loadMoreView.setLoadMorePadding(100);
        //RecyclerView的显示方式呈Listview的方式显示
        this.setLayoutManager(layoutManager);

        //当RecyclerView的列表没有数据时，所显示的布局
        this.setEmptyView(View.inflate(mActivity, R.layout.empty_view, null));

        //设置加载的脚布局
        this.setLoadMoreFooter(loadMoreView);
        this.getLoadMoreFooter().setOnDrawListener(new BaseLoadMoreView.OnDrawListener() {
            @Override
